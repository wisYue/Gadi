/*
  ...
  grid variables from FLASH (Uniform Grid) output files

  By Christoph Federrath, 2013-2020
...
void ComputeStructureFunctions(void)
{
    ...
    double grid_stag[MAX_NUM_BINS]; grid_stag[0] = D[X]/2.;
    ...
    while (grid[NumberOfBins] < 0.5*sqrt(3.0)*L[X]) /// max distance
    {
        ...
        grid_stag[NumberOfBins] = length + D[X]/2.;
        length += D[X];
    ...
    long long  *buf1_bin_counter_long           = new long long [NumBufferIndices];
    long long  *buf1_bin_counter_trsv           = new long long [NumBufferIndices];
    ...
    long long  *buf2_bin_counter_long           = new long long [NumBufferIndices];
    long long  *buf2_bin_counter_trsv           = new long long [NumBufferIndices];
    ...
    {
        int b1 = MyBlocks[ib];
        if (MyPE==0) cout<<" ["<<setw(6)<<MyPE<<"] working on block1 = "<<setw(6)<<ib+1<<" of "<<MyBlocks.size()<<endl;
        vector<double> bc1(3);
        ...
        for (int b2=0; b2<NBLK; b2++)
        {
            ...
            //if (MyPE==0) cout<<" ["<<setw(6)<<MyPE<<"] block1 = "<<setw(6)<<ib+1<<" of "<<MyBlocks.size()<<
            //                "; block2 count on root processor = "<<setw(6)<<b2+1<<" of "<<NBLK<<" done."<<endl;
            ...
            double rmin = LBlock[b2][X]/2., rmax = MaxLength;
            ...
            if (MyPE==0 && Debug) cout<<" ["<<setw(6)<<MyPE<<"] reading block2 done. block1 = "<<setw(6)<<ib+1<<" of "<<MyBlocks.size()<<
                ...
                for (long cellindex2=0; cellindex2<NBXYZ; cellindex2+=incr)
                {
                    ...
                    for (int i = 0; i < MaxStructureFunctionOrder; i++)
                      for (int t = 0; t < NumTypes; t++)
                      {
                        ...
                        if (incr_long_pow[t]/buf2_binsum_long[BufIndex] > numeric_epsilon)
                        {
                        ...
                        else // immediately reduce to buf1 and clear buf2
                        {
                            ...
                            buf2_bin_counter_long          [BufIndex] = 1;
                            buf2_binsum_long               [BufIndex] = incr_long_pow[t];
                        }
                        if (incr_trsv_pow[t]/buf2_binsum_trsv[BufIndex] > numeric_epsilon)
                        {
                            buf2_bin_counter_trsv[BufIndex]++;
                            buf2_binsum_trsv     [BufIndex] += incr_trsv_pow[t];
                        }
                        else // immediately reduce to buf1 and clear buf2
                        {
                            ...
                            buf2_bin_counter_trsv          [BufIndex] = 1;
                            buf2_binsum_trsv               [BufIndex] = incr_trsv_pow[t];
                        }
                      }
                    /// ==========================================
    ...
    long long *bin_counter_long           = new long long [NumBufferIndices];
    long long *bin_counter_trsv           = new long long [NumBufferIndices];
    ...
    for (int BufIndex = 0; BufIndex < NumBufferIndices; BufIndex++)
    {
        ...
        struc_funct_binsum_long   [BufIndex] = numeric_epsilon;
        struc_funct_binsum_trsv   [BufIndex] = numeric_epsilon;
        numeric_error_counter_long[BufIndex] = 0;
        numeric_error_counter_trsv[BufIndex] = 0;
    }

    /// Sum up CPU contributions
    MPI_Allreduce(buf1_bin_counter_long, bin_counter_long, NumBufferIndices, MPI_LONG_LONG_INT, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(buf1_bin_counter_trsv, bin_counter_trsv, NumBufferIndices, MPI_LONG_LONG_INT, MPI_SUM, MPI_COMM_WORLD);
    ...
    for (int BufIndex = 0; BufIndex < NumBufferIndices; BufIndex++)
    {
        if (bin_counter_long[BufIndex] > 0)
            struc_funct_binsum_long[BufIndex] /= static_cast<double>(bin_counter_long[BufIndex]);
        if (bin_counter_trsv[BufIndex] > 0)
            struc_funct_binsum_trsv[BufIndex] /= static_cast<double>(bin_counter_trsv[BufIndex]);
    }

    /// print out useful information and output
    if (MyPE==0)
    {
        long long ReSumCounter_long[NumTypes][MaxStructureFunctionOrder];
        long long ReSumCounter_trsv[NumTypes][MaxStructureFunctionOrder];
        for (int i = 0; i < MaxStructureFunctionOrder; i++)
          for (int t = 0; t < NumTypes; t++)
          {
